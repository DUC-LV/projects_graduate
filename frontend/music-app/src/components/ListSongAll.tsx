...
export const SongItem = ({ item }: props) => {
	...
	const addSongToPlaylist = useCallback(() => {
		if(item?.id && router?.query?.id){
			axiosInstances.post(`add-to-playlist/${router.query.id}`, { "song_id": item?.id}).then(res => {
				toast.success(res?.data?.msg);
			})
		}
	...
	return(
		<Grid item container alignItems={'center'}
			...
			<Grid item xs={4} sx={{ cursor: 'pointer', display: 'flex', justifyContent: 'end' }}>
				<AddOutlinedIcon sx={{ color: 'white' }} onClick={addSongToPlaylist}/>
			</Grid>
			<Popup
				isShow={isShow}
...
const ListSongAll = ({ data, description }: Props) => {
	const router = useRouter();
	return(
		<Grid container>
			<Grid item
